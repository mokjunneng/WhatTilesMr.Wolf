...
public class HexMap : MonoBehaviour {
    ...
    void Start () {
        GenerateMap();
        print("tiles that belong to player: " + player.GetComponent<Player>().tiles.Count);
	}
    ...
    private void GenerateMap() 
    {
        for (int column = 0; column < mapWidth; column++)
        {
            for (int row = 0; row < mapHeight; row++)
            {
                ...
                {
                    player.GetComponent<Player>().tiles.Add(hexGO);
                }
    ...
    //#region Temporarily not including in game
